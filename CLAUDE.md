...
# 环境操作限制
未经用户明确许可，禁止安装软件、工具链、编译器、rustup target、依赖等任何侵入性、影响本地开发环境的操作。本项目仅交叉编译到 `aarch64-unknown-linux-musl`（见下文构建部分），在 Windows 上无法原生 `cargo build` 通过（`std::os::unix`、`libc::statvfs`/`uname` 等平台限制），这是已知的正常现象，不要尝试通过安装工具链、切换 target 等方式"修复"，语法层面正确即可，实际编译验证以 CI 或用户手动操作为准。
...
- 设备开机由 `/home/root/loader.sh` 拉起（内容由 `config.rs` 管理），依次启动：
  1. `/home/root/ttyd/start.sh`（ttyd Web 终端，**独立于本项目的第三方二进制**，监听 7681）
     - 后端启动时会把该脚本的 ttyd 命令行校准为 `-i lo -b /api/terminal/proxy -H X-Remote-User`，
       ttyd 自身不做鉴权，改由后端代理注入信任头；运行中的实例不满足「代理模式 +
